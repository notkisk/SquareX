...
{
    public TextMeshProUGUI stateTimerText;
    ...
    void Update()
    {
        if (controlsOn==false)
        {
            if (stateTimerText == null) return;
            if (controlsDisabledTimer < 0f) stateTimerText.text = "0";
            else stateTimerText.text = controlsDisabledTimer.ToString("0.00");
    ...
    void FixedUpdate()
    {
        //Debug.Log(controlsDisabledTimer);
    ...
    */
    ...
    void Awake()
    {
        ...
        if (GameObject.Find("timer") != null)
            stateTimerText = GameObject.Find("timer").GetComponent<TextMeshProUGUI>();
